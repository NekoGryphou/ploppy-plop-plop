...
public sealed class HostUpdateServiceTests
{
    ...
    public async Task DownloadRequiresMatchingChecksumAndSignature()
    {
        ...
        Assert.IsTrue(path!.EndsWith("DeckyMyRig_Host__Windows_x64-1.3.0.exe", StringComparison.Ordinal));
        File.Delete(path);
    }
    ...
    public async Task DownloadDeletesAnInstallerWithAnInvalidPublisherSignature()
    {
        ...
        Assert.IsFalse(File.Exists(Path.Combine(Path.GetTempPath(), "DeckyMyRig_Host__Windows_x64-1.3.0.exe")));
    }
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            if (request.RequestUri!.AbsolutePath.EndsWith("release-manifest.json", StringComparison.Ordinal))
            {
                var json = JsonSerializer.Serialize(new
                {
                    ...
                    host = new
                    {
                        url = "https://github.com/NekoGryphou/decky-my-rig/releases/download/v1.3.0/DeckyMyRig_Host__Windows_x64.exe",
                        sha256 = checksum,
                    },
                });
