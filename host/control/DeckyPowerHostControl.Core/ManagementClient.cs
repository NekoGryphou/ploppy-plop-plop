...
{
    private const string DefaultPipeName = "DeckyPowerHostControl";
    ...
    private async Task<ManagementState> SendAsync(string operation, CancellationToken cancellationToken)
    {
        ...
        if (responseLength is 0 or > 65536) throw new IOException("DeckyPowerHost returned an invalid management response length.");
        ...
        return response ?? throw new IOException("DeckyPowerHost returned an empty management response.");
    }
}
