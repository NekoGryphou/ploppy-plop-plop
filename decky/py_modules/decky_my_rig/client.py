...
class HostClient:
    ...
    async def pair(self, device: Device, pairing_code: str) -> tuple[bytes, PairResponse]:
        ...
        if len(pairing_code) != 6 or not pairing_code.isascii() or not pairing_code.isdigit():
            raise HostError("pairing", "Enter the six-digit code shown by DeckyMyRigHostControl.")
        exchange = SPAKE2_A(
        ...
        if started.protocol_version != PROTOCOL_VERSION:
            raise HostError("protocol", "DeckyMyRigHost needs to be updated.")
        shared = exchange.finish(started.host_message)
        confirmation = hmac.digest(
            shared,
            b"deckymyrig-pairing-confirm-v1\0"
            + client_message
        ...
        key = HKDF(
            ...
            length=32,
            salt=None,
            info=b"deckymyrig-pairing-credential-v1",
        ).derive(shared)
        try:
    ...
    async def probe(self, device: Device) -> None:
        try:
            ...
            raise
        raise HostError("protocol", "DeckyMyRigHost accepted an unauthenticated status request.")
    ...
    async def _post(
        ...
        try:
            ...
            raise HostError(
                "unavailable",
                f"DeckyMyRigHost could not be reached at {device.address}:{device.port}.",
            ) from error
    ...
    def _verify_reply(reply, credential, authentication, path: str) -> None:
        ...
        try:
            ...
            raise HostError(
                "integrity",
                "DeckyMyRigHost returned a response that could not be authenticated.",
            ) from error
    ...
    def _decode_error(status: int, body: bytes) -> tuple[str, str]:
        try:
            ...
            message = "DeckyMyRigHost rejected the request."
