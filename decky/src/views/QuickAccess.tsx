...
*
* @returns Remote PC controls.
*/
export function QuickAccess(): JSX.Element {
  const { devices, statuses, error, act } = useDevices();
  if (devices.length === 0) return (
    <PanelSection title="Remote PCs"><PanelSectionRow><div><p>No PCs configured.</p><p>Add a PC using its address and MAC. It can be powered off and paired later.</p></div></PanelSectionRow><PanelSectionRow><ButtonItem layout="below" onClick={() => Navigation.Navigate("/decky-remote-power/settings")}>Add PC</ButtonItem></PanelSectionRow></PanelSection>
  );
