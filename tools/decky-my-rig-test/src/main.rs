...
async fn main() {
    if let Err(error) = run().await {
        eprintln!("decky-my-rig-test: {error}");
        std::process::exit(1);
    }
}
...
fn parse_arguments(mut arguments: impl Iterator<Item = String>) -> Result<Options, String> {
    ...
    let credential_file = values
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(".decky-my-rig-test.json"));
...
fn usage() -> String {
    "usage: decky-my-rig-test <pair|status|shutdown> --host HOST [--port PORT] [--code 483921] [--credential-file PATH]".into()
}
