...
- `proto/decky_power.proto`: canonical wire contract.
...
- `host/installer/DeckyPowerHost.iss`: service identity, ProgramData ACL, and
  Private-profile firewall rule.
- `decky/py_modules/decky_power/client.py`: production HTTP/Protobuf client,
  pairing, and mandatory success/error response verification.
- `decky/py_modules/decky_power/store.py`: separation and mode-0600 persistence
  of Deck-side credentials.
- `tools/decky-power-test/src/main.rs`: independent protocol implementation.
