...
$HostExecutable = Join-Path $ProjectDirectory "out\host\DeckyMyRigHost.exe"
$ClientExecutable = Join-Path $ProjectDirectory "tools\decky-my-rig-test\target\release\decky-my-rig-test.exe"
...
$listener.Stop()
$directory = Join-Path ([IO.Path]::GetTempPath()) ("decky-my-rig-windows-e2e-" + [Guid]::NewGuid().ToString("N"))
$config = Join-Path $directory "DeckyMyRigHost.toml"
